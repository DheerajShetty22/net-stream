...
const popularMoviesShow = (popmovies) => {
    movieEl.innerHTML = "";
    popmovies.forEach((pmovie) => {
        const { title, poster_path, vote_average } = pmovie;
        const poster = IMAGE_PATH + poster_path;
        const popmoviesDiv = document.createElement('div');
        popmoviesDiv.classList.add('carousel-cell')
        popmoviesDiv.innerHTML = `
        <img src="${poster}">
        <div class="item-overlay" id="ioverlay">
            <h4 class="pop-title"><span id="ispan">${title}</span></h4>
            <i class="fas fa-star">
                <h4 class="pop-rating">${vote_average}</h4>
            </i>
        </div>`
        movieEl.appendChild(popmoviesDiv);
    })
}
...
const popularTvshows = async () => {
    ...
    for (i = 0; i < tvpopdiv.length; i++) {
        ...
        if (res1.data.results[i].vote_average === 0) {
            Tvpoprating[i].innerText = "IMDB: N/A";
        }
        else {
