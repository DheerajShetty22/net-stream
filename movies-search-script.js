...
const TV_API_URL = "https://api.themoviedb.org/3/discover/movie?with_genres=10749&primary_release_year=2021&api_key=1fb03ee047ea180033951faef79393f2"
const DRAMA_MOVIE_API = "https://api.themoviedb.org/3/discover/movie?with_genres=18&primary_release_year=2021&api_key=1fb03ee047ea180033951faef79393f2"
const ACTION_MOVIE_API = "https://api.themoviedb.org/3/discover/movie?with_genres=16&primary_release_year=2021&api_key=1fb03ee047ea180033951faef79393f2&language=en-US&page=1"
const FICTION_MOVIE_API = "https://api.themoviedb.org/3/discover/movie?with_genres=27&primary_release_year=2018&api_key=1fb03ee047ea180033951faef79393f2"
...
button.addEventListener("click", () => {
    ...
    const MOVIE_SEARCH_API_URL = `https://api.themoviedb.org/3/search/movie?api_key=1fb03ee047ea180033951faef79393f2&language=en-US&page=1&include_adult=true&query=${searchInput}`;

    movieSearch(MOVIE_SEARCH_API_URL);
...
const movieSearch = async (MOVIE_SEARCH_API_URL) => {
...
const movies = (moviedata) => {
    ...
    moviedata.forEach((data) => {
        ...
        <div class="item-overlay" id="ioverlay${id}" onclick="getClassName(this.id)">
            <h4 class="pop-title"><span id="ispan">${title}</span></h4>
            <i class="fas fa-star">
...
const popularTvshows = async () => {
    ...
    for (i = 0; i < tvpopdiv.length; i++) {
        if (res1.data.results[i].poster_path == null) {
            ...
            Tvpoptitle[i].innerText = res1.data.results[i].title;
            if (res1.data.results[i].vote_average === 0) {
                Tvpoprating[i].innerText = "N/A";
            }
            else {
